  ...
  return (
    <div
      ...
      className={`plan-step cursor-pointer p-4 border rounded transition-all flex items-start space-x-3.5 select-none ${
        isDone
          ? 'bg-[#161a26]/40 border-[#202534] opacity-60'
          : 'bg-[#11141d] border-[#202534] hover:border-[#818cf8]'
      }`}
    >
      <div
        className={`w-5 h-5 rounded border flex items-center justify-center mt-0.5 transition-colors ${
          isDone
            ? 'bg-[#818cf8] border-[#818cf8] text-[#090a0f]'
            : 'bg-[#090a0f] border-[#202534] text-transparent'
        }`}
      >
      ...
      <div className="flex-1 space-y-1">
        <div className="flex items-center justify-between">
          <h4
            className={`text-sm font-semibold tracking-tight ${
              isDone ? 'line-through text-[#8b94a8]' : 'text-[#f3f4f8]'
            }`}
          >
            {step.title}
          </h4>
          {step.week_label && (
            <span className="text-[10px] font-mono uppercase text-[#818cf8] bg-[#818cf8]/10 px-2 py-0.5 rounded border border-[#818cf8]/30 flex items-center space-x-1">
        ...
        {step.detail && (
          <p className="text-xs text-[#cbd2e1] leading-relaxed font-sans">
            {step.detail}
          </p>
        )}
      </div>
